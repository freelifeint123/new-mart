...
export function FirstLoginCredentialsDialog() {
  ...
  //   - their account is still on the seeded defaults, AND
  //   - they have not dismissed the popup for this session.
  const open = useMemo(
    () =>
      !!state.accessToken &&
  ...
  // Prefill the username field with the admin's current handle whenever
  // the popup re-opens for a new session.
  useEffect(() => {
    if (open) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!wantsUsernameChange && !wantsPasswordChange) {
      setFormError("Update your username, password, or both — or click Skip for now.");
      return;
    }
    ...
    setSubmitting(true);
    try {
      // Order matters: rotate the password first so the username PATCH
      // runs against the freshly issued access token. (changePassword
      // returns the new token AND mirrors it into auth state, so the
      // subsequent fetch picks it up automatically.)
      if (wantsPasswordChange) {
      ...
      toast({
        ...
        description: wantsPasswordChange
          ? "Use your new credentials on next login."
          : "Use your new username on next login.",
      });
      // After a successful change the API has already cleared
      // `defaultCredentials`, so the dialog closes naturally.
    } catch (err) {
      setFormError(err instanceof Error ? err.message : "Failed to update credentials.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Dialog
      ...
      onOpenChange={(next) => {
        ...
        // "Skip for now" — defaults stay valid.
        if (!next) handleSkip();
      }}
    >
