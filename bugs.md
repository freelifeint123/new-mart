...
- **Status**: [COMPLETED] — Verified the actual rendering paths: CommandPalette's `Highlight` component renders text via JSX children (`<span>{text.slice(...)}</span>`, `<mark>{text.slice(...)}</mark>`) — React auto-escapes JSX text children, so a search query like `<script>` displays as literal text and never hits the DOM as markup. No `dangerouslySetInnerHTML` is used. `lib/format.ts` returns plain strings and never emits HTML — the new `formatDateLocale` is a pure `Intl.DateTimeFormat` wrapper. The shared `escapeHtml()` from `lib/escapeHtml.ts` is reserved for the few sites that legitimately need `dangerouslySetInnerHTML` (Leaflet/Mapbox marker labels via `sanitizeMarkerHtml`, chart CSS variables via `isSafeCssIdent`/`isSafeCssColor`) — see "Potential XSS Risk" and "Potential XSS in Chart Configurations".
